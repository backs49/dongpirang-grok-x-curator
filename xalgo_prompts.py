...
- 모든 아이디어가 비슷한 패턴(특히 질문형으로 끝나는 것)이 되지 않게 **다양한 스타일**로 구성하세요.
- Reply 유도를 위한 질문형 CTA는 **강제하지 말고**, 자연스럽게 필요할 때만 사용하세요.
- 이미지 첨부는 **선택적**으로만 추천하세요. 매번 강제하지 마세요.
- {current_date_kr} 기준으로 시의성 있는 최신 트렌드를 자연스럽게 반영하되, 억지로 AI 트렌드를 끌어오지 마세요.

## 허용되는 다양한 포스트 스타일 (5개 중 최소 3개는 서로 다른 스타일을 사용하세요)
- 가치 제공형 / 실용 팁형
- 공감·경험 공유형
- 인사이트·분석형
- 비교·선택형
- 질문 없이도 자연스럽게 대화를 유발하는 부드러운 마무리형
- "이 포스트가 도움이 되셨다면 리포스트 부탁드려요" 같은 가벼운 요청형

## X Algorithm 최적화 전략
- Reply와 Repost를 자연스럽게 유도하되, 과도한 질문형은 피하세요.
- 실용적 가치 + 공감 + 시의성이 조화된 콘텐츠가 현재 가장 강력합니다.
- 이미지나 스크린샷이 있으면 좋지만, 없어도 충분히 좋은 포스트가 될 수 있게 작성하세요.
